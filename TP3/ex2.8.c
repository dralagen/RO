...
printf("z = %lf\n",z);
for(i = 0;i < NBVILLE;i++) printf("X%c = %d, ",'a'+i,(int)(x[i] + 0.5)); /* un cast est ajouté, x[i] pourrait être égal à 0.99999... */
puts("");
for(i = NBVILLE;i < NBVAR;i++) printf("Y%c = %d, ",'a'+i,(int)(x[i] + 0.5)); /* un cast est ajouté, x[i] pourrait être égal à 0.99999... */
puts("");
